...
int doDijkstraDiameter (int graph[N_POINTS][N_POINTS], int path[], int size, int src){
	int index;
...
int doDiameter (int graph[N_POINTS][N_POINTS], int size){
	int distMax = -1,dist;
	int src;

	for (int i=0;i<size;i++){
		src = i;
		dist = doDijkstraDiameter (graph, path, size, src);
...
int main(){
	...
	for (i=0;i<N_TESTS;i++){
		...
		result = doDiameter (graph,N_POINTS);
