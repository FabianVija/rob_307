...
int findIndex(int dist[], bool dSet[],int size){
	int ip_size;
	int flag = true;
	...
	for (int i=0;i<size;i+=N_IP_POINTS){
		...
		Xil_DCacheFlushRange((u32)&ip_index, sizeof(int));
		...
		Xil_DCacheInvalidateRange((u32) &ip_index, sizeof(int));

		if(dist[i+ip_index]<distMin){
			indexMin = i+ip_index;
...
void updateDistPath(int graph[N_POINTS][N_POINTS],int path[],int dist[], bool dSet[],int size,int src,int index){
	int ip_size;
	int flag = false;
	int aux[3];
	...
	for (int i=0;i<size;i+=N_IP_POINTS){
		...
		// Update Cash
		Xil_DCacheFlushRange((u32)aux, 3*sizeof(int));
		Xil_DCacheFlushRange((u32)&dSet[i], ip_size*sizeof(int));
		Xil_DCacheFlushRange((u32)&dist[i], ip_size*sizeof(int));
		Xil_DCacheFlushRange((u32)&path[i], ip_size*sizeof(int));
		Xil_DCacheFlushRange((u32)&graph[index][i], ip_size*sizeof(int));
		Xil_DCacheFlushRange((u32)&index, sizeof(int));

		// Send data
		XAxiDma_SimpleTransfer(&axiDMA, (u32)aux, 3*sizeof(int),XAXIDMA_DMA_TO_DEVICE );
		XAxiDma_SimpleTransfer(&axiDMA, (u32)&dSet[i], ip_size*sizeof(int),XAXIDMA_DMA_TO_DEVICE );
		XAxiDma_SimpleTransfer(&axiDMA, (u32)&dist[i], ip_size*sizeof(int),XAXIDMA_DMA_TO_DEVICE );
