...
int result2[N_POINTS+1];
float centroids[N_CLUSTER][N_FEATURES];
int in_index[N_CLUSTER];

float data_ip[N_POINTS_IP][N_FEATURES];
float cent_ip[N_CLUSTER][N_FEATURES];
...
int kmeans(){
	...
	for(int i=0; i<sizeof(in_index); i++){
		printf("%d, ", in_index[i]);
	}
	...
	while(end == 0){
		...
		while(counter < N_POINTS){
			...
			XAxiDma_SimpleTransfer(&axiDMA, (u32) data_ip, N_POINTS_IP*N_FEATURES*sizeof(float),XAXIDMA_DEVICE_TO_DMA );
			while(XAxiDma_Busy(&axiDMA, XAXIDMA_DEVICE_TO_DMA));

			XAxiDma_SimpleTransfer(&axiDMA, (u32) cent_ip, N_CLUSTER*N_FEATURES*sizeof(float),XAXIDMA_DEVICE_TO_DMA );
			while(XAxiDma_Busy(&axiDMA, XAXIDMA_DEVICE_TO_DMA));

			Xil_DCacheInvalidateRange((u32) result, (N_POINTS+1)*sizeof(int));
			Xil_DCacheInvalidateRange((u32) data_ip, N_POINTS_IP*N_FEATURES*sizeof(float));
			Xil_DCacheInvalidateRange((u32) cent_ip, N_CLUSTER*N_FEATURES*sizeof(float));


			end = result[N_POINTS];

			for(int i=counter ; i<counter+N_POINTS_IP+1 ; i++){
				printf("%d, ", result[i]);
			}
			printf("\n");
			for(int i=0 ; i<N_POINTS+1 ; i++){
