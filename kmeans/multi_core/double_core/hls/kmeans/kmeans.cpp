...
volatile int results[N_POINTS_IP];
volatile int endip[2];
...
#pragma HLS INTERFACE s_axilite port=return bundle=CRTL_BUS
	...
	read_res:for(int i=0 ; i<N_POINTS_IP ; i++){
		intSdCh valIn = inStream.read();
	...
	if(gain == 3){
		send2:for(int idx = 0; idx < 2; idx++){

			intSdCh valOut;
			valOut.data = endip[idx];
			valOut.keep = valref.keep;
			valOut.strb = valref.strb;
			valOut.user = valref.user;
			valOut.last = (idx==1)? 1:0;
			valOut.id = valref.id;
			valOut.dest = valref.dest;
			outStream << valOut;
		}

		write_c: for(int i=0 ; i<N_CLUSTER ; i++){
			for(int j= 0; j<N_FEATURES ; j++){
				intSdCh valOut;
				union{int valo ; float valin ;} converter;
				converter.valin = centroids[i][j];

				valOut.data = converter.valo;
				valOut.keep = valref.keep;
				valOut.strb = valref.strb;
				valOut.user = valref.user;
